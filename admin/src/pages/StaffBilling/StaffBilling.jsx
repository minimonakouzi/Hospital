...
import LookupSelect, { patientLabel } from "../../components/LookupSelect/LookupSelect";
...
function Select({ value, options, onChange }) {
  return <select className={selectClass} value={value} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option}>{option}</option>)}</select>;
}

function Stat({ label, value }) {
  return <div className="rounded-3xl border border-[#dbe6f7] bg-white p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">{label}</p><p className="mt-2 text-2xl font-bold text-blue-700">{value}</p></div>;
}
...
function BillForm({ form, setForm, onSubmit, saving, close, title, lookups, lookupLoading, lookupError }) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 px-3 py-4 sm:items-center"><form onSubmit={onSubmit} className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"><div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5"><div><h2 className="text-xl font-bold text-slate-950">{title}</h2></div><button type="button" onClick={close}><X /></button></div>{lookupError ? <div className="mx-5 mt-4 rounded-2xl border border-rose-100 bg-rose-50 p-3 text-sm font-medium text-rose-700">{lookupError}</div> : null}<div className="overflow-y-auto p-5"><div className="grid gap-4 md:grid-cols-2"><LookupSelect label="Patient" required allowEmpty={false} value={form.patientId} onChange={(v) => setForm((p) => ({ ...p, patientId: v }))} options={lookups.patients} getLabel={patientLabel} loading={lookupLoading} /><label className="admin-field">Bill Type<Select value={form.billType} options={createTypes} onChange={(billType) => setForm((p) => ({ ...p, billType }))} /></label><Field label="Description" value={form.description} onChange={(v) => setForm((p) => ({ ...p, description: v }))} /><Field label="Discount" type="number" value={form.discount} onChange={(v) => setForm((p) => ({ ...p, discount: v }))} /><Field label="Tax" type="number" value={form.tax} onChange={(v) => setForm((p) => ({ ...p, tax: v }))} /><label className="admin-field">Payment Status<Select value={form.paymentStatus} options={createPaymentStatuses} onChange={(paymentStatus) => setForm((p) => ({ ...p, paymentStatus }))} /></label><Field label="Payment Method" value={form.paymentMethod} onChange={(v) => setForm((p) => ({ ...p, paymentMethod: v }))} /><Field label="Paid Amount" type="number" value={form.paidAmount} onChange={(v) => setForm((p) => ({ ...p, paidAmount: v }))} /><Field label="Insurance Provider" value={form.insuranceProvider} onChange={(v) => setForm((p) => ({ ...p, insuranceProvider: v }))} /><Field label="Policy Number" value={form.insurancePolicyNumber} onChange={(v) => setForm((p) => ({ ...p, insurancePolicyNumber: v }))} /><label className="admin-field">Insurance Status<Select value={form.insuranceStatus} options={createInsuranceStatuses} onChange={(insuranceStatus) => setForm((p) => ({ ...p, insuranceStatus }))} /></label><Field label="Coverage Amount" type="number" value={form.insuranceCoverageAmount} onChange={(v) => setForm((p) => ({ ...p, insuranceCoverageAmount: v }))} /><Field label="Due Date" type="date" value={form.dueDate} onChange={(v) => setForm((p) => ({ ...p, dueDate: v }))} /><label className="admin-field md:col-span-2">Notes<textarea className="admin-textarea min-h-20" value={form.notes || ""} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} /></label></div><div className="mt-5 rounded-3xl border border-[#dbe6f7] bg-[#f8fbff] p-4"><div className="flex items-center justify-between"><h3 className="font-semibold">Invoice Items</h3><button type="button" onClick={addItem} className="rounded-2xl bg-white px-3 py-2 text-xs font-semibold text-blue-700 ring-1 ring-blue-100">Add Item</button></div><div className="mt-4 grid gap-3">{(form.items || []).map((item, index) => <div key={index} className="grid gap-3 rounded-2xl bg-white p-3 md:grid-cols-[1.4fr_0.6fr_0.8fr_0.8fr_auto]"><input className={inputClass} placeholder="Item title" value={item.title || ""} onChange={(e) => updateItem(index, "title", e.target.value)} /><input className={inputClass} type="number" min="0" placeholder="Qty" value={item.quantity || 0} onChange={(e) => updateItem(index, "quantity", e.target.value)} /><input className={inputClass} type="number" min="0" placeholder="Unit price" value={item.unitPrice || 0} onChange={(e) => updateItem(index, "unitPrice", e.target.value)} /><div className="flex h-11 items-center rounded-2xl bg-[#f8fbff] px-4 text-sm font-semibold text-slate-700">{money(preview.items[index]?.total)}</div><button type="button" onClick={() => removeItem(index)} className="h-11 rounded-2xl bg-rose-50 px-3 text-xs font-semibold text-rose-700">Remove</button></div>)}</div><div className="mt-4 grid gap-3 rounded-2xl bg-white p-4 text-sm font-medium text-slate-700 sm:grid-cols-4"><div>Subtotal <span className="block font-semibold text-blue-700">{money(preview.subtotal)}</span></div><div>Total <span className="block font-semibold text-blue-700">{money(preview.total)}</span></div><div>Patient Payable <span className="block font-semibold text-blue-700">{money(preview.payable)}</span></div><div>Estimated Due <span className="block font-semibold text-rose-700">{money(preview.due)}</span></div></div></div></div><div className="flex justify-end gap-3 border-t border-slate-100 p-5"><button type="button" onClick={close} className="h-11 rounded-2xl border px-4 font-semibold">Cancel</button><button disabled={saving} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-blue-600 px-4 font-semibold text-white">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Save</button></div></form></div>;
}

function Field({ label, value, onChange, required = false, type = "text" }) {
  return <label className="admin-field">{label}<input type={type} required={required} className={inputClass} value={value || ""} onChange={(e) => onChange(e.target.value)} /></label>;
}

function SmallModal({ title, children, onSubmit, close, saving }) {
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 px-3 py-4 sm:items-center"><form onSubmit={onSubmit} className="w-full max-w-xl rounded-3xl bg-white p-5 shadow-2xl"><div className="flex justify-between"><h2 className="text-xl font-bold">{title}</h2><button type="button" onClick={close}><X /></button></div><div className="mt-5 grid gap-4">{children}</div><div className="mt-5 flex justify-end gap-3"><button type="button" onClick={close} className="h-11 rounded-2xl border px-4 font-semibold">Cancel</button><button disabled={saving} className="h-11 rounded-2xl bg-blue-600 px-4 font-semibold text-white">Save</button></div></form></div>;
}
...
export function BillingWorkspace({ title = "Billing", subtitle, api, header = "staff", authMode = "staff", getToken }) {
  const { lookups, lookupLoading, lookupError } = useSavedLookups({ authMode, getToken });
  ...
  const content = <div className="px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto grid max-w-7xl gap-5"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6"><Stat label="Total Bills" value={summary.total} /><Stat label="Paid" value={summary.paid} /><Stat label="Unpaid" value={summary.unpaid} /><Stat label="Partial" value={summary.partial} /><Stat label="Revenue" value={money(summary.revenue)} /><Stat label="Outstanding" value={money(summary.outstanding)} /></section><section className="rounded-3xl border border-[#dbe6f7] bg-white p-4 shadow-sm"><div className="grid gap-3 xl:grid-cols-[1fr_170px_170px_190px]"><div className="relative"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input className={`${inputClass} pl-11`} value={filters.search} onChange={(e) => setFilters((p) => ({ ...p, search: e.target.value }))} placeholder="Search bills..." /></div><Select value={filters.billType} options={billTypes} onChange={(billType) => setFilters((p) => ({ ...p, billType }))} /><Select value={filters.paymentStatus} options={paymentStatuses} onChange={(paymentStatus) => setFilters((p) => ({ ...p, paymentStatus }))} /><Select value={filters.insuranceStatus} options={insuranceStatuses} onChange={(insuranceStatus) => setFilters((p) => ({ ...p, insuranceStatus }))} /></div></section>{message.text ? <Notice message={message} close={() => setMessage({ type: "", text: "" })} /> : null}<section className="overflow-hidden rounded-3xl border border-[#dbe6f7] bg-white shadow-sm">{loading ? <div className="p-14 text-center text-sm font-semibold text-blue-700"><Loader2 className="mr-2 inline h-5 w-5 animate-spin" />Loading bills...</div> : bills.length === 0 ? <div className="p-14 text-center"><CreditCard className="mx-auto h-12 w-12 text-blue-700" /><h2 className="mt-3 font-semibold">No billing records found.</h2></div> : <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-100 text-sm"><thead className="bg-[#f8fbff] text-xs uppercase tracking-[0.12em] text-slate-500"><tr><th className="px-5 py-4 text-left">Invoice</th><th className="px-5 py-4 text-left">Patient</th><th className="px-5 py-4 text-left">Amounts</th><th className="px-5 py-4 text-left">Status</th><th className="px-5 py-4 text-left">Insurance</th><th className="px-5 py-4 text-left">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{bills.map((bill) => <tr key={idOf(bill)} className="align-top"><td className="px-5 py-4"><p className="font-semibold text-blue-700">{bill.billCode || "Invoice"}</p><p className="mt-1 font-medium text-slate-950">{bill.billType}</p><p className="text-xs text-slate-500">{date(bill.invoiceDate)}</p></td><td className="px-5 py-4">{personName(bill.patientId)}</td><td className="px-5 py-4"><p>Total {money(bill.totalAmount)}</p><p>Paid {money(bill.paidAmount)}</p><p className="font-semibold text-rose-700">Due {money(bill.dueAmount)}</p></td><td className="px-5 py-4"><Badge>{bill.paymentStatus}</Badge></td><td className="px-5 py-4"><p>{bill.insuranceProvider || "-"}</p><Badge>{bill.insuranceStatus}</Badge></td><td className="px-5 py-4"><div className="flex flex-wrap gap-2"><button onClick={() => setSelected(bill)} className="rounded-xl bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700"><Eye className="mr-1 inline h-3.5 w-3.5" />View</button><button onClick={() => printInvoice(bill)} className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-semibold text-white"><Printer className="mr-1 inline h-3.5 w-3.5" />Print</button><button onClick={() => openEdit(bill)} className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700"><Pencil className="mr-1 inline h-3.5 w-3.5" />Edit</button><button onClick={() => openPayment(bill)} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700"><CreditCard className="mr-1 inline h-3.5 w-3.5" />Payment</button><button onClick={() => openInsurance(bill)} className="rounded-xl bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />Insurance</button><button onClick={() => remove(bill)} className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700"><Trash2 className="mr-1 inline h-3.5 w-3.5" />Delete</button></div></td></tr>)}</tbody></table></div>}</section></div>{modal === "bill" ? <BillForm title={form._id ? "Edit Bill" : "Create Bill"} form={form} setForm={setForm} onSubmit={saveBill} saving={saving} close={() => setModal("")} lookups={lookups} lookupLoading={lookupLoading} lookupError={lookupError} /> : null}{modal === "payment" ? <SmallModal title="Update Payment" onSubmit={savePayment} close={() => setModal("")} saving={saving}><label className="admin-field">Payment Status<Select value={form.paymentStatus} options={createPaymentStatuses} onChange={(paymentStatus) => setForm((p) => ({ ...p, paymentStatus }))} /></label><Field label="Payment Method" value={form.paymentMethod} onChange={(v) => setForm((p) => ({ ...p, paymentMethod: v }))} /><Field label="Paid Amount" type="number" value={form.paidAmount} onChange={(v) => setForm((p) => ({ ...p, paidAmount: v }))} /></SmallModal> : null}{modal === "insurance" ? <SmallModal title="Update Insurance" onSubmit={saveInsurance} close={() => setModal("")} saving={saving}><Field label="Insurance Provider" value={form.insuranceProvider} onChange={(v) => setForm((p) => ({ ...p, insuranceProvider: v }))} /><Field label="Policy Number" value={form.insurancePolicyNumber} onChange={(v) => setForm((p) => ({ ...p, insurancePolicyNumber: v }))} /><label className="admin-field">Insurance Status<Select value={form.insuranceStatus} options={createInsuranceStatuses} onChange={(insuranceStatus) => setForm((p) => ({ ...p, insuranceStatus }))} /></label><Field label="Coverage Amount" type="number" value={form.insuranceCoverageAmount} onChange={(v) => setForm((p) => ({ ...p, insuranceCoverageAmount: v }))} /></SmallModal> : null}{selected ? <Detail bill={selected} close={() => setSelected(null)} /> : null}</div>;
