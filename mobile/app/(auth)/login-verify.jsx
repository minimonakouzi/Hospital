...
export default function LoginVerifyScreen() {
    ...
    try {
      ...
      if (result.status === "complete" && result.createdSessionId) {
        ...
        Alert.alert(
          ...
          "The code was accepted but the session is not complete yet.",
        );
      }
    } catch (err) {
      const message =
      ...
      console.log("LOGIN VERIFY ERROR:", JSON.stringify(err, null, 2));
    } finally {
      setLoading(false);
    }
  };
  ...
  const onResendPress = async () => {
    ...
    try {
      ...
      setCode("");
      Alert.alert("Code resent", "A new login code was sent to your email.");
    } catch (err) {
      const message =
        err?.errors?.[0]?.longMessage ||
        err?.errors?.[0]?.message ||
      ...
      console.log("LOGIN RESEND ERROR:", JSON.stringify(err, null, 2));
    } finally {
      setResending(false);
    }
  };
